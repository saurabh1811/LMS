...
from app.services.models import (TokenServiceRequest,TokenServiceResponse, 
								...
								UserServiceRequest, UserServiceResponse, TokenEmailServiceRequest,
...
from app.db.models import (LoginDbResponseModel,LoginDbRequestModel, SignupDbRequestModel
							...
							UserDbRequest, UpdateUserDbRequest, LoginEmailDbRequestModel)
...
class TokenService:
	...
	def GoogleLogin(self, db: Session, request: TokenEmailServiceRequest)->TokenServiceResponse:
		LoginDbRequest:LoginEmailDbRequestModel=LoginEmailDbRequestModel(google_user=request.google_user)
		db_response = dbManager().get_email_user(db, LoginDbRequest)
		service_response= DbConverter().ToServiceModel(db_response)      
		return service_response

	def Signup(self, db:Session, request: SignupServiceRequest)->SignupServiceResponse:
		print (SignupServiceRequest)
		service_response:SignupServiceResponse=SignupServiceResponse(user=SignedUser(username="",email="",user_id="",hashed_password="",mobile="",first_name="",last_name="",schoolId=""))
