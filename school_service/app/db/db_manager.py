...
from app.db.models import (DbUserInDB, LoginDbResponseModel,LoginDbRequestModel, 
							...
							,UserListDbResponse, ResetPasswordDbRequest,
...
class dbManager:
	...
	def add_user(self, db: Session, request: SignupDbRequestModel)->SignupDbResponseModel:
		response: SignupDbResponseModel = SignupDbResponseModel(user=DbUserInDB( hashed_password="", email="",  password="", confirm_password="",mobile="",school_name="",board="",address=""))
		...
		db.add(db_obj)
		db.commit()
		response.user= DbUserInDB( hashed_password=hashed_password, email= request.user.email, user_id= db_obj.id, school_name= db_obj.school_name, board= db_obj.board, guid= str(db_obj.guid), password=request.user.password, confirm_password=request.user.password,mobile=request.user.mobile,address=request.user.address)
		return response 
